...
class _BookingFormState extends State<BookingForm> {
  ...
  void _loadAvailablePromos() {
    ...
    if (serviceName.contains('potong') || serviceName.contains('rambut')) {
      ...
      if (serviceName.contains('perawatan') || serviceName.contains('creambath') || 
          serviceName.contains('vitamin') || serviceName.contains('masker')) {
        promos.add({
    ...
    if (serviceName.contains('facial') || serviceName.contains('wajah') || 
    ...
    if (serviceName.contains('makeup') || serviceName.contains('tata rias') || 
      ...
      if (promo.isNotEmpty) {
        switch (promo['type']) {
          ...
          case 'free_service':
            ...
            discountAmount = promo['value'].toDouble();
            finalPrice = originalPrice; // Harga tetap sama, tapi dapat bonus layanan gratis
            break;
        }
      }
    }
  ...
  void _submitBooking() {
    if (_formKey.currentState!.validate()) {
      ...
      // Create booking object with complete data
      final booking = {
        'title': widget.title,
        'original_price': _formatPrice(originalPrice),
        'final_price': _formatPrice(finalPrice),
        'discount_amount': discountAmount > 0 ? _formatPrice(discountAmount) : null,
        'promo_used': selectedPromo,
        'promo_type': selectedPromo != null ? availablePromos.firstWhere(
          (p) => p['name'] == selectedPromo,
          orElse: () => {},
        )['type'] : null,
        'date': '${selectedDate.day}/${selectedDate.month}/${selectedDate.year}',
        'formatted_date': '${selectedDate.day.toString().padLeft(2, '0')} ${_getMonthName(selectedDate.month)} ${selectedDate.year}',
